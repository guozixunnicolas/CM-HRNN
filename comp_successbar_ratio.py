...
import subprocess
import math
from model import decode_rhythm
def get_empty_dict():
    dictionary= {
        "note_tmp":[],
        "rhythm_tmp":[],
...
def get_feature(mid_file, long_short_threshold = 16):
    try:
        out = subprocess.run(["java", "-jar","omnisia.jar","-i",mid_file,"-max","4"],stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout= 30)
    except :
...
folders_2b_evaluated =["test/generated_result/01_16_2021_16_59_04_merged_3t_fc/120000/16/0.7_0.2"]
merged_dict=[]
...
#df.to_csv("new_model_diff_emb_max4.csv")
